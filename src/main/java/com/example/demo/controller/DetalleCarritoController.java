...
public class DetalleCarritoController {
    ...
    public ResponseEntity<?> createDetalleCarrito(
        ...
        if (detalleCarrito.getProducto() == null || detalleCarrito.getProducto().getProd_id() == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Producto no especificado.");
        }
        ...
        if (detalleCarrito.getCantidad() > stockDisponible) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("No hay stock suficiente para el producto.");
        }
